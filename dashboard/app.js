...
function timeline() {
  ...
  <nav class="abs timeline-sections"><button class="${state.timelineMode === 'photographer' ? 'active' : ''}" data-timeline-mode="photographer">Photographer</button><button class="${state.timelineMode === 'videographer' ? 'active' : ''}" data-timeline-mode="videographer">Videographer</button><button class="submit-timeline">Submit The Timeline</button></nav>
  <label class="abs same-timeline"><input type="checkbox" ${state.videographerSame ? 'checked' : ''} data-same-video> Videographer follows photographer timeline</label>
...
function slot(s, i) { const short = slotTooShort(s[0], s[1]); return `<article class="slot ${state.selectedSlot === i ? 'selected' : ''}" data-slot="${i}"><button class="slot-click" data-slot="${i}" aria-label="Edit ${s[2]}"></button><div class="abs slot-time ${short ? 'short' : 'ok'}"><span>${s[0]}</span><i></i><span>${s[1]}</span></div><div class="abs slot-main"><h3>${s[2]}</h3><p>${s[3]}</p>${s[4]?`<p>${s[4]}</p>`:''}</div><div class="abs slot-actions">${s[5]?'<button class="details-btn" data-slot="'+i+'">details</button>':''}</div></article>`; }
function timeOptions() { return ['01:00 PM','01:15 PM','01:30 PM','01:45 PM','02:00 PM','02:15 PM','02:30 PM','02:45 PM','03:00 PM','03:15 PM','03:30 PM','03:45 PM','04:00 PM','04:15 PM','04:30 PM','04:45 PM','05:00 PM','05:15 PM','05:30 PM','05:45 PM','06:00 PM','07:00 PM','08:00 PM','09:00 PM','10:00 PM','11:00 PM','12:00 AM'].map(t => `<option value="${t}">${t}</option>`).join(''); }
...
function editor() { const s = selectedTimelineSlot(); return `<aside class="abs editor"><h2>${s[2]} <button class="edit-title" type="button" aria-label="Edit timeslot title">✎</button></h2><div class="warning" role="alert">You have selected less time than recommended</div><div class="note-box"><strong>NOTE</strong><p>Our photographer will capture your getting ready photos, which will include your dress, shoes, details. Please be sure to have your invitations, rings, bouquet and anything else you would like photographed set aside for our team. We recommend at least 1 hour total for getting ready.</p></div><form id="timeline-form"><label><span class="field-label">Timeslot title</span><input class="input" name="title" value="${s[2]}"></label><label><span class="field-label">Define time</span><div class="time-picker"><span>From</span><select class="input" name="from"><option value="${s[0]}">${s[0]}</option>${timeOptions()}</select><span>To</span><select class="input" name="to"><option value="${s[1]}">${s[1]}</option>${timeOptions()}</select></div></label><label><span class="field-label">Location name</span><input class="input" name="place" list="venue-suggestions" placeholder="Restaurant, hotel, venue" data-place-name><datalist id="venue-suggestions"><option value="The Plaza Hotel"><option value="Brooklyn Botanic Garden"><option value="The River Cafe"><option value="501 Union"><option value="The Foundry"></datalist></label><label><span class="field-label">Location address</span><div class="address-grid"><input class="input" name="address1" list="address-suggestions" placeholder="Address 1" data-address-one><input class="input" name="address2" placeholder="Address 2"></div><datalist id="address-suggestions"><option value="768 5th Ave, New York, NY 10019"><option value="990 Washington Ave, Brooklyn, NY 11225"><option value="1 Water St, Brooklyn, NY 11201"><option value="501 Union St, Brooklyn, NY 11231"></datalist></label><div class="address-grid"><label><span class="field-label">City</span><input class="input" name="city"></label><label><span class="field-label">Zip code</span><input class="input" name="zip"></label></div><label><span class="field-label">Additional notes</span><textarea class="input" name="note" placeholder="Please write if there is anything else you want us to know">${s[4] || ''}</textarea></label><div class="editor-actions"><button type="button" class="delete" data-delete-selected>delete timeslot</button><span>or</span><button type="button" class="blue-btn save-btn">Save changes</button></div></form></aside>`; }
...
function bind() {
  ...
  document.querySelectorAll('[data-timeline-mode]').forEach(b => b.addEventListener('click', () => { state.timelineMode = b.dataset.timelineMode; render(); }));
  document.querySelector('[data-same-video]')?.addEventListener('change', e => { state.videographerSame = e.target.checked; });
...
function checkTimelineDuration() { const form = document.querySelector('#timeline-form'); const warning = document.querySelector('.warning'); if (!form || !warning) return; warning.classList.toggle('show', slotTooShort(form.from.value, form.to.value)); }
function saveSelectedSlot() { const form = document.querySelector('#timeline-form'); if (!form) return; const slot = selectedTimelineSlot(); slot[0] = form.from.value || slot[0]; slot[1] = form.to.value || slot[1]; slot[2] = (form.title.value || slot[2]).toUpperCase(); slot[3] = `${slot[0]} - ${slot[1]} | ${form.place.value || form.address1.value || 'Please define location'}`; slot[4] = form.note.value || ''; checkTimelineDuration(); render(); }
