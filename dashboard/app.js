...
function timeline() {
  ...
  <nav class="abs timeline-sections"><button>Photographer Timeline</button><button>Videographer</button><button>Submit The Timeline</button></nav>
  <button class="abs add-slot" aria-label="Add another time slot" data-add-slot>+</button><section class="abs slots">${state.timeline.map(slot).join('')}</section>${editor()}`;
}
