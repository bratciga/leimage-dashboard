const state = {
  ...
  teamNameIndex: null,
  confirmAction: null,
  teamMembers: ['Photographer', 'Photographer'],
  shortSlots: [],
  vendors: [
...
function timeline() {
  const activeTeamIndex = Number(String(state.timelineMode).replace('team-', '')) || 0;
  const modeLabel = `${state.teamMembers[activeTeamIndex] || 'Photographer'} Timeline`;
  ...
  <nav class="abs timeline-sections"><div class="team-head"><span>Team</span><button type="button" class="team-add" aria-label="Add team member" data-add-team-member>+</button></div>${teamButtons}${activeTeamIndex > 0 ? '<button type="button" class="delete team-delete" data-delete-team-member><span class="delete-x">×</span><span class="delete-text">delete team member</span></button>' : ''}<button class="submit-timeline">Submit The Timeline</button></nav>
...
function slot(s, i) { const hasInfo = Boolean(s[3] && !String(s[3]).toLowerCase().includes('please define')); const short = state.shortSlots.includes(i); const status = hasInfo ? (short ? 'bad' : 'complete') : 'empty'; return `<article class="slot ${state.selectedSlot === i ? 'selected' : ''} slot-${status}" data-slot="${i}"><button class="slot-click" data-slot="${i}" aria-label="Edit ${s[2]}"></button><div class="slot-node"><i></i></div><div class="abs slot-time ${status}"><span>${s[0]}</span><i></i><span>${s[1]}</span></div><div class="abs slot-main" draggable="true" data-drag-slot="${i}"><h3>${s[2]}</h3><p>${s[3]}</p>${s[4]?`<p>${s[4]}</p>`:''}</div><div class="abs slot-actions">${s[5]?'<button class="details-btn" data-slot="'+i+'">details</button>':''}</div></article>`; }
...
function bind() {
  ...
  document.querySelector('[data-add-slot]')?.addEventListener('click', () => { const insertAt = Math.min(state.selectedSlot + 1, state.timeline.length); state.timeline.splice(insertAt, 0, ['0:00', '0:00', 'NEW TIMESLOT', 'Please define time', '', false]); state.selectedSlot = insertAt; render(); });
  ...
  document.querySelector('[data-team-name-form]')?.addEventListener('submit', e => { e.preventDefault(); const name = e.currentTarget.teamName.value.trim(); if (!name) return; if (state.teamNameIndex !== null) { state.teamMembers[state.teamNameIndex] = name; state.timelineMode = `team-${state.teamNameIndex}`; } else { state.teamMembers.push(name); state.timelineMode = `team-${state.teamMembers.length - 1}`; } state.teamNamePopup=false; state.teamNameIndex=null; state.linkPopup=false; render(); });
  ...
  document.querySelector('[data-delete-team-member]')?.addEventListener('click', e => { e.preventDefault(); e.stopPropagation(); const index = Number(String(state.timelineMode).replace('team-', '')) || 0; if (index <= 0 || state.teamMembers.length <= 1) return; state.confirmAction = {type:'team', index}; state.teamNamePopup=false; state.linkPopup=false; render(); });
  document.querySelectorAll('[data-link-team]').forEach(b => b.addEventListener('click', e => { e.stopPropagation(); state.linkPopup = !state.linkPopup; render(); }));
